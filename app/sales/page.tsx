...
useEffect(() => {
  ...
  filteredLeads.sort((a, b) => {
    ...
    if (isCallbackOverdue(a) && !isCallbackOverdue(b)) return -1;
    if (!isCallbackOverdue(a) && isCallbackOverdue(b)) return 1;

    if (isCallbackDue(a) && !isCallbackDue(b)) return -1;
    if (!isCallbackDue(a) && isCallbackDue(b)) return 1;
  ...
  }, [currentIndex, searchedLeads.length]);

  const today = new Date().toLocaleDateString("en-CA");

  const todaysCallbacks = leads.filter((lead) => {
    return (
  ...
  const priorityLeads = leads.filter((lead) => lead.priority);
  ...
  const todaysCalls = allActivities.filter(
    (activity) => activity.type === "status"
  ).length;

  const todaysInterested = allActivities.filter(
    (activity) =>
      activity.type === "status" &&
      activity.text.toLowerCase().includes("interessiert")
  ).length;

  const todaysNotReached = allActivities.filter(
    (activity) =>
      activity.type === "status" &&
      (
        activity.text.toLowerCase().includes("nicht_erreicht") ||
        activity.text.toLowerCase().includes("nicht erreicht")
      )
  ).length;

  const todaysCallbackActivities = allActivities.filter(
    (activity) => activity.type === "callback"
  ).length;
  ...
  async function updateLead(status: string) {
        ...
        ? [
            {
              lead_id: currentLead.ID,
              type: "callback",
              text: `Rückruf gesetzt für ${callbackDate} um ${callbackTime}`,
            },
          ]
        : []),
    ...
    setAllActivities((prev) => [...activityItems, ...prev]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-8 flex items-center justify-between">
          ...
          <div className="flex gap-6">
            <div className="text-right">
              ...
              <p className="text-2xl font-bold">
                {currentIndex + 1} / {searchedLeads.length}
              </p>
            </div>
        ...
        <div className="mb-3">
          <button
            ...
            `}
          >
            {showTeam ? "Team-Ansicht aktiv" : "Nur meine Pipeline"}
          </button>
        </div>
